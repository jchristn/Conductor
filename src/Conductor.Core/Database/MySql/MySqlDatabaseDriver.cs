namespace Conductor.Core.Database.MySql
{
    ...
    public class MySqlDatabaseDriver : DatabaseDriverBase
    {
        ...
        public MySqlDatabaseDriver(DatabaseSettings settings)
        {
            ...
            EndpointGroup = new Conductor.Core.Database.EndpointGroupMethods(this, Conductor.Core.Database.RequestAnalyticsSqlDialect.MySql);
            ...
            VirtualModelRunnerReservation = new Conductor.Core.Database.VirtualModelRunnerReservationMethods(this, Conductor.Core.Database.RequestAnalyticsSqlDialect.MySql);
        ...
        /// <inheritdoc/>
        protected override string TelemetryDatabaseSystem => "mysql";
        ...
        private async Task RunMigrationsAsync(CancellationToken token = default)
        {
            ...
            await EnsureColumnAsync("requesthistory", "selectionstrategy", "ALTER TABLE requesthistory ADD COLUMN selectionstrategy VARCHAR(64);", token).ConfigureAwait(false);
            await EnsureColumnAsync("requesthistory", "endpointgroupguid", "ALTER TABLE requesthistory ADD COLUMN endpointgroupguid VARCHAR(64);", token).ConfigureAwait(false);
            await EnsureColumnAsync("requesthistory", "endpointgroupname", "ALTER TABLE requesthistory ADD COLUMN endpointgroupname VARCHAR(255);", token).ConfigureAwait(false);
            await EnsureColumnAsync("requesthistory", "backoffreason", "ALTER TABLE requesthistory ADD COLUMN backoffreason VARCHAR(128);", token).ConfigureAwait(false);
            await EnsureColumnAsync("requesthistory", "adaptiveselection", "ALTER TABLE requesthistory ADD COLUMN adaptiveselection TINYINT(1) NOT NULL DEFAULT 0;", token).ConfigureAwait(false);
            await EnsureColumnAsync("requesthistory", "policyfallbackused", "ALTER TABLE requesthistory ADD COLUMN policyfallbackused TINYINT(1) NOT NULL DEFAULT 0;", token).ConfigureAwait(false);
            ...
            await EnsureColumnAsync("requesthistory", "reservationwindowstartutc", "ALTER TABLE requesthistory ADD COLUMN reservationwindowstartutc DATETIME;", token).ConfigureAwait(false);
            await EnsureColumnAsync("requesthistory", "reservationwindowendutc", "ALTER TABLE requesthistory ADD COLUMN reservationwindowendutc DATETIME;", token).ConfigureAwait(false);
            ...
            await EnsureColumnAsync("requestanalyticsevents", "reservationwindowstartutc", "ALTER TABLE requestanalyticsevents ADD COLUMN reservationwindowstartutc DATETIME;", token).ConfigureAwait(false);
            await EnsureColumnAsync("requestanalyticsevents", "reservationwindowendutc", "ALTER TABLE requestanalyticsevents ADD COLUMN reservationwindowendutc DATETIME;", token).ConfigureAwait(false);
