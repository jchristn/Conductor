namespace Conductor.Core.Database.PostgreSql.Implementations
{
    ...
    /// <summary>
    /// PostgreSQL QoS profile methods implementation. A profile is persisted as an aggregate across the
    ...
    public class QosProfileMethods : IQosProfileMethods
    {
        ...
        /// <summary>
        /// Instantiate the PostgreSQL QoS profile methods implementation.
        /// </summary>
        /// <param name="driver">Database driver. Must not be null.</param>
        public QosProfileMethods(PostgreSqlDatabaseDriver driver)
        {
        ...
        public async Task<QosProfile> ReadDefaultAsync(string tenantId, CancellationToken token = default)
        {
            ...
            string query = "SELECT * FROM qosprofiles WHERE tenantid = '" + _Driver.Sanitize(tenantId) + "' AND isdefault = " + _Driver.FormatBoolean(true) + " LIMIT 1;";
            return await AssembleFromQueryAsync(query, token).ConfigureAwait(false);
        }
