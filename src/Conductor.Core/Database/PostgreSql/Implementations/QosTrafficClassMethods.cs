namespace Conductor.Core.Database.PostgreSql.Implementations
{
    ...
    /// <summary>
    /// PostgreSQL QoS traffic class methods implementation.
    /// </summary>
    public class QosTrafficClassMethods : IQosTrafficClassMethods
    {
        ...
        /// <summary>
        /// Instantiate the PostgreSQL QoS traffic class methods implementation.
        /// </summary>
        /// <param name="driver">Database driver. Must not be null.</param>
        public QosTrafficClassMethods(PostgreSqlDatabaseDriver driver)
        {
        ...
        public async Task<QosTrafficClass> ReadByNameAsync(string tenantId, string name, CancellationToken token = default)
        {
            ...
            string query = "SELECT * FROM qostrafficclasses WHERE tenantid = '" + _Driver.Sanitize(tenantId) + "' AND name = '" + _Driver.Sanitize(name) + "' LIMIT 1;";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
            if (result == null || result.Rows.Count < 1) return null;
            return QosTrafficClass.FromDataRow(result.Rows[0]);
        }
        ...
        public async Task<EnumerationResult<QosTrafficClass>> EnumerateAsync(string tenantId, EnumerationRequest request, CancellationToken token = default)
        {
            ...
            string query = "SELECT * FROM qostrafficclasses " + whereClause + " ORDER BY name ASC LIMIT " + (request.MaxResults + 1) + " OFFSET " + offset + ";";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
