namespace Conductor.Core.Database.PostgreSql.Implementations
{
    ...
    /// <summary>
    /// PostgreSQL request history methods implementation.
    /// </summary>
    public class RequestHistoryMethods : IRequestHistoryMethods
    {
        ...
        public async Task<RequestHistorySummaryResult> GetSummaryAsync(RequestHistorySummaryFilter filter, CancellationToken token = default)
        {
            ...
            {
                case "minute":
                    dateTrunc = "date_trunc('minute', createdutc)";
                    break;
                case "15minute":
                    dateTrunc = "to_timestamp(floor(extract(epoch from createdutc) / 900) * 900)";
                    break;
                case "6hour":
                    dateTrunc = "to_timestamp(floor(extract(epoch from createdutc) / 21600) * 21600)";
                    break;
                case "day":
                    dateTrunc = "date_trunc('day', createdutc)";
                    break;
                default:
                    dateTrunc = "date_trunc('hour', createdutc)";
                    break;
            }
