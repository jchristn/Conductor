namespace Conductor.Core.Database.PostgreSql.Implementations
{
    ...
    public class UserMethods : IUserMethods
    {
        ...
        public async Task<UserMaster> ReadByEmailAsync(string tenantId, string email, CancellationToken token = default)
        {
            ...
            string query = "SELECT * FROM users WHERE tenantid = '" + _Driver.Sanitize(tenantId) + "' AND email = '" + _Driver.Sanitize(email) + "' AND active = TRUE;";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
        ...
        public async Task<EnumerationResult<UserMaster>> EnumerateAsync(string tenantId, EnumerationRequest request, CancellationToken token = default)
        {
            ...
            {
                conditions.Add("(firstname ILIKE '%" + _Driver.Sanitize(request.NameFilter) + "%' OR lastname ILIKE '%" + _Driver.Sanitize(request.NameFilter) + "%' OR email ILIKE '%" + _Driver.Sanitize(request.NameFilter) + "%')");
            }
