namespace Conductor.Core.Database.PostgreSql.Implementations
{
    ...
    /// <summary>
    /// PostgreSQL virtual model runner methods implementation.
    /// </summary>
    public class VirtualModelRunnerMethods : IVirtualModelRunnerMethods
    {
        ...
        public VirtualModelRunnerMethods(PostgreSqlDatabaseDriver driver)
        {
        ...
        {
            string query = "SELECT * FROM virtualmodelrunners WHERE active = TRUE;";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
