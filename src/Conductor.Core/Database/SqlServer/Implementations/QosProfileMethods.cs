namespace Conductor.Core.Database.SqlServer.Implementations
{
    ...
    /// <summary>
    /// SQL Server QoS profile methods implementation. A profile is persisted as an aggregate across the
    ...
    public class QosProfileMethods : IQosProfileMethods
    {
        ...
        /// <summary>
        /// Instantiate the SQL Server QoS profile methods implementation.
        /// </summary>
        /// <param name="driver">Database driver. Must not be null.</param>
        public QosProfileMethods(SqlServerDatabaseDriver driver)
        {
        ...
        public async Task<QosProfile> ReadDefaultAsync(string tenantId, CancellationToken token = default)
        {
            ...
            string query = "SELECT TOP 1 * FROM qosprofiles WHERE tenantid = '" + _Driver.Sanitize(tenantId) + "' AND isdefault = 1;";
            return await AssembleFromQueryAsync(query, token).ConfigureAwait(false);
        }
        ...
        public async Task<EnumerationResult<QosProfile>> EnumerateAsync(string tenantId, EnumerationRequest request, CancellationToken token = default)
        {
            ...
            string query = "SELECT * FROM qosprofiles " + whereClause + " ORDER BY name ASC" +
