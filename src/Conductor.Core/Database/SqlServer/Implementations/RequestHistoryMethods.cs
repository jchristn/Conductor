namespace Conductor.Core.Database.SqlServer.Implementations
{
    ...
    /// <summary>
    /// SQL Server request history methods implementation.
    /// </summary>
    public class RequestHistoryMethods : IRequestHistoryMethods
    {
        ...
        public async Task<RequestHistorySummaryResult> GetSummaryAsync(RequestHistorySummaryFilter filter, CancellationToken token = default)
        {
            ...
            {
                case "minute":
                    dateTrunc = "DATEADD(MINUTE, DATEDIFF(MINUTE, 0, createdutc), 0)";
                    break;
                case "15minute":
                    dateTrunc = "DATEADD(MINUTE, (DATEDIFF(MINUTE, 0, createdutc) / 15) * 15, 0)";
                    break;
                case "6hour":
                    dateTrunc = "DATEADD(HOUR, (DATEDIFF(HOUR, 0, createdutc) / 6) * 6, 0)";
                    break;
                case "day":
                    dateTrunc = "CAST(CAST(createdutc AS DATE) AS DATETIME)";
                    break;
                default:
                    dateTrunc = "DATEADD(HOUR, DATEDIFF(HOUR, 0, createdutc), 0)";
                    break;
            }
