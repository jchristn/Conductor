namespace Conductor.Core.Database.SqlServer.Implementations
{
    ...
    /// <summary>
    /// SQL Server user methods implementation.
    /// </summary>
    public class UserMethods : IUserMethods
    {
        ...
        public async Task<UserMaster> ReadByEmailAsync(string tenantId, string email, CancellationToken token = default)
        {
            ...
            string query = "SELECT * FROM users WHERE tenantid = '" + _Driver.Sanitize(tenantId) + "' AND email = '" + _Driver.Sanitize(email) + "' AND active = 1;";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
        ...
        public async Task<EnumerationResult<UserMaster>> EnumerateAsync(string tenantId, EnumerationRequest request, CancellationToken token = default)
        {
            ...
            {
                conditions.Add("(firstname LIKE '%" + _Driver.Sanitize(request.NameFilter) + "%' OR lastname LIKE '%" + _Driver.Sanitize(request.NameFilter) + "%' OR email LIKE '%" + _Driver.Sanitize(request.NameFilter) + "%')");
            }
        ...
        private string GetOrderBy(EnumerationOrderEnum order)
        {
            switch (order)
            {
                ...
                case EnumerationOrderEnum.NameDescending:
                    return "ORDER BY lastname DESC, firstname DESC";
                default:
                    return "ORDER BY createdutc DESC";
            }
        }
