namespace Conductor.Core.Database.Sqlite.Implementations
{
    ...
    /// <summary>
    /// SQLite virtual model runner methods implementation.
    /// </summary>
    public class VirtualModelRunnerMethods : IVirtualModelRunnerMethods
    {
        ...
        public VirtualModelRunnerMethods(SqliteDatabaseDriver driver)
        {
        ...
        public async Task<VirtualModelRunner> CreateAsync(VirtualModelRunner vmr, CancellationToken token = default)
        {
            ...
            string query = "INSERT INTO virtualmodelrunners (id, tenantid, name, hostname, basepath, apitype, loadbalancingmode, loadbalancingpolicyid, modelaccesspolicyid, modelrunnerendpointids, modelconfigurationids, modeldefinitionids, modelconfigurationmappings, timeoutms, allowembeddings, allowcompletions, allowmodelmanagement, strictmode, sessionaffinitymode, sessionaffinityheader, sessiontimeoutms, sessionmaxentries, requesthistoryenabled, active, createdutc, lastupdateutc, labels, tags, metadata) " +
        ...
        public async Task<VirtualModelRunner> ReadByBasePathAsync(string basePath, CancellationToken token = default)
        {
            ...
            string query = "SELECT * FROM virtualmodelrunners WHERE LOWER(RTRIM(basepath, '/')) = '" + _Driver.Sanitize(normalizedPath) + "' AND active = 1;";
            DataTable result = await _Driver.ExecuteQueryAsync(query, false, token).ConfigureAwait(false);
