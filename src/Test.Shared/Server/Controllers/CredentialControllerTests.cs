namespace Test.Shared.Server.Controllers
{
    ...
    public class CredentialControllerTests : ControllerTestBase
    {
        ...
        public async Task Create_WithEmptyBearerToken_GeneratesUniqueToken()
        {
            Credential credential = new Credential
            {
        ...
        public async Task Read_WithNullTenantId_ReadsCredentialByGlobalId()
        {
            Credential created = await _Controller.Create(TestTenantId, TestUserId, new Credential { Name = "Global Read Credential" });

            Credential result = await _Controller.Read(null, created.Id);
        ...
        public async Task Update_WithEmptyBearerToken_PreservesExistingToken()
        {
            Credential created = await _Controller.Create(TestTenantId, TestUserId, new Credential());
            ...
            result.BearerToken.Should().Be(created.BearerToken);
        }
