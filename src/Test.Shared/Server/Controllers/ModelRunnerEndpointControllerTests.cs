namespace Test.Shared.Server.Controllers
{
    ...
    /// </summary>
    public class ModelRunnerEndpointControllerTests : ControllerTestBase
    {
        ...
        public async Task Delete_RemovesEndpointFromVmrReferences()
        {
            ...
            {
                Name = "Endpoint Owner",
                BasePath = "/v1.0/api/owner/",
                ModelRunnerEndpointIds = new List<string> { endpoint.Id }
            });

            await _Controller.Delete(TestTenantId, endpoint.Id);
            VirtualModelRunner updated = await _VmrController.Read(TestTenantId, vmr.Id);

            updated.ModelRunnerEndpointIds.Should().NotContain(endpoint.Id);
        }
        public async Task Delete_RemovesEndpointFromMultipleVmrReferences()
        {
            ModelRunnerEndpoint endpoint = await _Controller.Create(TestTenantId, new ModelRunnerEndpoint
            {
                Name = "Shared Endpoint",
            ...
            VirtualModelRunner vmr1 = await _VmrController.Create(TestTenantId, new VirtualModelRunner
            {
                Name = "Endpoint Owner 1",
                BasePath = "/v1.0/api/owner-one/",
                ModelRunnerEndpointIds = new List<string> { endpoint.Id }
            });

            VirtualModelRunner vmr2 = await _VmrController.Create(TestTenantId, new VirtualModelRunner
            {
                Name = "Endpoint Owner 2",
                BasePath = "/v1.0/api/owner-two/",
                ModelRunnerEndpointIds = new List<string> { endpoint.Id, "mre_other" }
            });

            await _Controller.Delete(TestTenantId, endpoint.Id);

            (await _VmrController.Read(TestTenantId, vmr1.Id)).ModelRunnerEndpointIds.Should().NotContain(endpoint.Id);
            VirtualModelRunner updated = await _VmrController.Read(TestTenantId, vmr2.Id);
            updated.ModelRunnerEndpointIds.Should().NotContain(endpoint.Id);
            updated.ModelRunnerEndpointIds.Should().Contain("mre_other");
        }
        ...
        public async Task GetHealth_WithNoHealthCheckService_ThrowsException()
        {
            ModelRunnerEndpoint endpoint = await _Controller.Create(TestTenantId, new ModelRunnerEndpoint
            {
                Name = "Test Endpoint",
